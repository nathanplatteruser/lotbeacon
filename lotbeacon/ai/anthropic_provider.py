    ...
    def draft(self, ctx: DraftContext) -> str:
        ...
        r = self.client.messages.create(
            ...
            messages=[{"role": "user", "content": (
                ...
                f"Verified appointment slots you may offer (offer exactly these two as a choice; never invent others): {json.dumps(ctx.slots or [])}\n"
                f"Clarification to raise naturally (if any): {ctx.clarify or 'none'}\n"
                f"CONFIRMED appointment (only if present may you say 'see you'): {json.dumps(ctx.appointment)}\n"
